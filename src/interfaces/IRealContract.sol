...
interface IRealContract {
    // 案件相關事件
    event CaseAdded(uint256 indexed caseNum, string caseName, string caseDescription);
    event CaseStaked(uint256 indexed caseNum, address indexed participant, uint256 amount);
    ...
    event FeeRateUpdated(uint256 stakeCompensationFeeRate, uint256 executeCaseFeeRate);
